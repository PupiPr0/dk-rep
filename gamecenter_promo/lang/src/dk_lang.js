(async function() {
  ...
  const translations = await loadJSON(`https://res.dkon.app/gamecenter_promo/lang/src/${chosen}.json`)
    || await loadJSON('https://res.dkon.app/gamecenter_promo/lang/src/en.json') || {};

  // декодер (на случай если в JSON экранированы &lt;br&gt;)
  ...
  function applyTranslations(map) {
    ...
    nodes.forEach(node => {
      ...
      // Вариант A: показывать HTML (браузер обработает <br>)
      node.innerHTML = decodeHtml(value);

      // Вариант B (более безопасный): заменить <br> на \n и показать с переносами
      // node.textContent = decodeHtml(value).replace(/<br\s*\/?>/gi, '\n');
